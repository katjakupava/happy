...
if page == pages[2] : 
    ...
    st.image(image)
    st.write("Firstly, we select from each database only the data corresponding to the period 2007-2018. Then we unified the name of the columns and verified that the countries in each data set had the same name, for which we used geopandas to unify the name of the countries."
"We merge all the data sets into one and observing a loss of information in the process, the final size of the data set is 965x18. Finally, the data was split for training and testing purposes, and missing values were imputed using the KNN imputer and numerical features were scaled using Standard Scaler to ensure data integrity.")
    
    st.subheader('**Model building**',divider='rainbow')
    st.write("After finishing data preparation part, multiple machine learning algorithms, were used to predict the happiness score. Next, aiming to find the best hyper-parameters, Grid search is used which is a process that uses an exhaustive search to find the best values from a manually specified subset of hyper-parameters of the target algorithm.")
    st.write("The data used is quantitative, the following machine learning algorithms used to predict the happiness score:")
    ...
    tabla = pd.DataFrame([
            {"Set":"Training","Model": "Multiple Linear Model", "Accuracy":LR.score(X_train, y_train).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train, y_pred_train_LR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train, y_pred_train_LR).round(2)},
            {"Set":"Test","Model": "Multiple Linear Model", "Accuracy":LR.score(X_test, y_test).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test, y_pred_LR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test, y_pred_LR).round(2)},
            {"Set":"Training","Model": "KNeighbors Regressor Model", "Accuracy":KNN.score(X_train, y_train).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train, y_pred_train_KNN).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train, y_pred_train_KNN).round(2)},
            {"Set":"Test","Model": "KNeighbors Regressor Model", "Accuracy":KNN.score(X_test, y_test).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test, y_pred_KNN).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test, y_pred_KNN).round(2)},
            {"Set":"Training","Model": "Decision Tree Regressor", "Accuracy":DTR.score(X_train, y_train).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train, y_pred_train_DTR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train, y_pred_train_DTR).round(2)},
            {"Set":"Test","Model": "Decision Tree Regressor", "Accuracy":DTR.score(X_test, y_test).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test, y_pred_DTR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test, y_pred_DTR).round(2)},
            {"Set":"Training","Model": "Random Forest Regressor", "Accuracy":RFR.score(X_train, y_train).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train, y_pred_train_RFR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train, y_pred_train_RFR).round(2)},
            {"Set":"Test","Model": "Random Forest Regressor", "Accuracy":RFR.score(X_test, y_test).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test, y_pred_RFR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test, y_pred_RFR).round(2)},
            {"Set":"Training","Model": "Gradient Boosting Regressor", "Accuracy":GBR.score(X_train, y_train).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train, y_pred_train_GBR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train, y_pred_train_GBR).round(2)},
            {"Set":"Test","Model": "Gradient Boosting Regressor", "Accuracy":GBR.score(X_test, y_test).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test, y_pred_GBR).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test, y_pred_GBR).round(2)},
            ])
      
    edited_df = st.data_editor(tabla,hide_index=True)

    st.write("Considering the difference in data types and sizes, each model will have different performance characteristics."
        "However, after comparing and exploring the performance of the evaluated models, we can say that the **Randon Forest Regressor** model performed better than most and that the Random Forest regressor algorithms are well suited to a regression problem that is based on a dataset published in the context of quality of life at the country level. ")

    st.write("The purpose of feature selection in machine learning is to determine the best set of variables to build effective models of the studied phenomena. Feature classification is used to understand the importance of variables. The results for Models found are presented")

    choice = ['Decision Tree Regressor', 'Random Forest Regressor','Gradient Boosting Regressor']
    ...
    st.write('In order to optimize the model, and evaluate whether there is an optimal fit with only the four most important variables, we reduce the data set, taking into account only the following variables:')
    st.write('- Log GDP')
    st.write('- Social Supp')
    st.write('- Life Expectancy') 
    st.write('- Gini')
    st.write('')
    st.write('and we proceed again to fit a **Random Forest Regressor Model**, obtaining the following results:')
...
#numerical variables
    ...
    tabla_R = pd.DataFrame([
            {"Set":"Training", "Accuracy":RFR.score(X_train_R, y_train_R).round(2) , "Error - MAE":metrics.mean_absolute_error(y_train_R, y_pred_train_RFR_R).round(2) ,'Error - MSE':metrics.mean_squared_error(y_train_R, y_pred_train_RFR_R).round(2)},
            {"Set":"Test", "Accuracy":RFR.score(X_test_R, y_test_R).round(2) , "Error - MAE":metrics.mean_absolute_error(y_test_R, y_pred_RFR_R).round(2) ,'Error - MSE':metrics.mean_squared_error(y_test_R, y_pred_RFR_R).round(2)},
            ]) 
    edited_df = st.data_editor(tabla_R,hide_index=True)
    ...
    st.write('From the results we observe that there is no significant improvement between the initial model and the reduced model (with only the four most important variables).')

    st.subheader('**Model Optimatization**',divider='rainbow')

    #Random Hyperparameter Grid
    #To use RandomizedSearchCV, we must first create a parameter grid to sample during fitting:
    ...
    st.write('We first determine the hyperparameters available in the Randon Forest Regressor model and examine their default values, then a random search instance is created, since in the random search algorithm not all combinations are tested, but it selects randomly to sample a wide range of values, reducing processing time. '
    'The most important arguments in RandomizedSearchCV are n_iter, which controls the number of different combinations to test, and cv, which is the number of folds to be used for cross-validation (we use 100 and 3 respectively).')
   ...
   # rf_random.fit(X_train_R, y_train_R)
    ...
    best_random=RandomForestRegressor(n_estimators= 800,min_samples_split= 2,min_samples_leaf= 2,max_features= 'sqrt',max_depth= 50,bootstrap= False)
    ...
    best_shap = RandomForestRegressor(n_estimators= 800,min_samples_split= 2,min_samples_leaf= 2,max_features= 'sqrt',max_depth= 50,bootstrap= False)
    ...
    st.write("Accuracy Test: ",best_shap.score(X_test, y_test).round(2))
    ...
    #shap_interaction_values = shap.TreeExplainer(best_shap).shap_interaction_values(X.iloc[:2000,:])
